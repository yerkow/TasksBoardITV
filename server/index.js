...
io.on("connection", (socket) => {
  ...
  socket.on("authenticate", (token) => {
    try {
      ...
      console.log(
        `✅ Пользователь ${decoded.userId} аутентифицирован через WebSocket`,
      );
...
const broadcastOnlineUsers = async () => {
  try {
    ...
    console.log(
      "📡 Отправлен обновленный список пользователей онлайн:",
      usersWithStatus.filter((u) => u.isOnline).length,
      "из",
...
const notifyAll = (event, data) => {
  console.log(
    `📡 WebSocket broadcast: ${event} to ${io.engine.clientsCount} clients`,
  );
  io.emit(event, data);
};
...
const notifyUser = (userId, event, data) => {
  ...
  if (userSocketIds && userSocketIds.size > 0) {
    console.log(
      `📡 WebSocket notify user ${userId}: ${event} to ${userSocketIds.size} sockets`,
    );
    ...
    console.log(
      `⚠️ User ${userId} not found in active sockets for event: ${event}`,
    );
  }
};
...
app.put("/api/users/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ error: "Недостаточно прав для обновления пользователя" });
    }
...
app.get("/api/users/status", authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Ошибка при получении статуса пользователей" });
  }
});
...
app.post("/api/tasks", authenticateToken, async (req, res) => {
  try {
    const { title, description, priority, deadline, assigneeId, assigneeName } =
      req.body;
...
app.put("/api/tasks/:id", authenticateToken, async (req, res) => {
  try {
    ...
    console.log(
      "📡 Отправка WebSocket уведомления task_updated для задачи:",
      task.id,
      "статус:",
      taskWithRussianLabels.status,
    );
  ...
  async (req, res) => {
    try {
      ...
      if (reportType === "text" && textContent) {
        ...
        return res
          .status(400)
          .json({ error: "Не предоставлен ни файл, ни текстовый контент" });
      }
      ...
      console.log(
        "📡 Отправка WebSocket уведомления task_updated для загрузки отчета, задача:",
        task.id,
        "статус:",
        taskWithRussianLabels.status,
      );
      ...
      res.json({
        task: taskWithRussianLabels,
        fileUrl: reportFile.url || null,
      });
    } catch (error) {
...
const startServers = () => {
  ...
  server.listen(HTTP_PORT, "0.0.0.0", () => {
    ...
    console.log(
      `📊 API доступно по адресу: http://${localIP}:${HTTP_PORT}/api`,
    );
